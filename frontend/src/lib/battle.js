...
const WIRED_EFFECTS = new Set([
  ...
  "escalating_dot", "team_shield", "revive_once", "lifesteal",
  "stun_chance", "freeze_chance", "poison_dot", "burn_dot",
]);
...
export function resolveOnHitEffects(actor, target, jutsu) {
  ...
  // Additional signature passives — these are intentionally lightweight but
  // immediately make heroes play differently in battle.
  const passiveType = actor.passive?.effect_type;
  if (passiveType === "lifesteal") {
    const heal = Math.max(1, Math.round(actor.atk * 0.10));
    actor.hp = Math.min(actor.maxHp, actor.hp + heal);
    events.push(makeEvent("PASSIVE_TRIGGER", { actorUid: actor.uid, text: `Lifesteal +${heal}` }));
  }
  const passiveStatus = { stun_chance: "stun", freeze_chance: "freeze", poison_dot: "poison", burn_dot: "burn" }[passiveType];
  if (passiveStatus && Math.random() < (passiveStatus === "stun" || passiveStatus === "freeze" ? 0.22 : 0.35)) {
    const existing = target.statuses.find((s) => s.effectType === passiveStatus && s.source === actor.uid);
    if (!existing) {
      const status = { id: `passive_${passiveStatus}_${actor.uid}_${target.uid}_${Date.now()}`, effectType: passiveStatus, source: actor.uid, duration: passiveStatus === "stun" || passiveStatus === "freeze" ? 1 : 3 };
      if (passiveStatus === "poison" || passiveStatus === "burn") status.magnitude = Math.max(1, Math.round(actor.atk * 0.24));
      target.statuses.push(status);
      events.push(makeEvent("PASSIVE_TRIGGER", { actorUid: actor.uid, targetUid: target.uid, text: `${actor.name}'s passive: ${passiveStatus}!` }));
    }
  }

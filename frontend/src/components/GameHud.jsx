...
const Pill = ({ icon: Icon, color, value, testid }) => (
  <div
    data-testid={testid}
    className="pointer-events-auto flex items-center gap-1.5 pl-1.5 pr-3 py-1 rounded-full bg-white/80 backdrop-blur-md border border-black/10"
...
export default function GameHud() {
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-40 pointer-events-none" data-testid="game-hud">
      <div className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-[#F4EEE2] via-[#F4EEE2]/70 to-transparent" />
      <div className="relative max-w-6xl mx-auto px-3 sm:px-5 pt-[calc(0.55rem+env(safe-area-inset-top))] flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-2 pointer-events-auto">
          <div className="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full bg-white/80 backdrop-blur-md border border-black/10" style={{ boxShadow: "inset 0 1px 0 rgba(255,255,255,0.7)" }} data-testid="hud-player">
          ...
          <button onClick={handleLogout} data-testid="hud-logout" className="pointer-events-auto w-8 h-8 rounded-full bg-white/80 backdrop-blur-md border border-black/10 flex items-center justify-center text-slate-500 hover:text-fox hover:border-fox/40 transition-colors">

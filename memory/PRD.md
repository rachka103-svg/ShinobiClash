...
## Implemented (2026-08-18 — Visual redesign Phase A+B: Theme system + Roster)
- **Phase A (theme system)**: new `frontend/src/lib/theme.js` — centralized BG/SURFACE/TEXT/ACCENT/RARITY/ELEMENT/RADIUS/SPACE/TYPE tokens + `glow()`/`scrimBottom()`/`vignetteInset()` helpers. `lib/styles.js` now re-exports `RARITY`/`ELEMENT` from `theme.js` unchanged — every existing screen keeps working exactly as before (purely additive).
- **Phase B (HeroPortrait + Roster)**: new `frontend/src/components/HeroPortrait.jsx` — the primary reusable artwork-first hero card (compact/featured/detail modes). Artwork fills ~85%+ of the card; name/rarity/element/level are overlaid directly on the artwork via a bottom scrim + minimal glyphs — no separate boxed footer. Rarity-tinted outer glow scales with rarity tier; UR/LR get an elite shine sweep.
- Rewrote `Roster.jsx`'s header/grid: removed the old top inventory-strip (dashboard-y, duplicated info already shown per-hero in the detail dialog); added a slim divider-separated stat strip (Heroes/Best rarity/Squad Power/Crystals — no boxes) + minimal rarity/element filter pill rows; grid now renders `HeroPortrait` (2 cols on mobile → 5 on desktop) instead of the old `NinjaCard`. The existing hero detail dialog was intentionally left untouched (that's Phase C, next).
- **Bug caught during mobile-viewport inspection** (not just "code compiles"): the animation wrapper around each grid card broke CSS Grid's default child-stretch, collapsing cards to near-invisible dots — fixed by making `HeroPortrait`'s root and its wrapper explicitly `w-full`.
- Verified on a real 390×844 mobile viewport: cards render large and portrait-dominant, filters work and correctly show an empty state, hero detail dialog still opens correctly, no horizontal page overflow.
- **Flagged, not fixed (out of scope this round per explicit instruction not to touch navigation)**: the existing mobile nav (`TopBar.jsx`'s `md:hidden` row) shows all ~9 tabs as a cramped horizontally-scrollable strip on real phone widths — doesn't cause page-level overflow, but is visually dense. Earmarked for the project's own planned "Phase F — Navigation" polish pass.
- NinjaCard/Gallery/TeamBuilder/Summon/Admin/Lobby/Campaign/Spire/Arena/Battle — all untouched, no backend changes.
